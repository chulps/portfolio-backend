...
router.get('/', auth, async (req, res) => {
  try {
    const user = await User.findById(req.user.id)
      .populate('friends', 'username email profileImage name')  // Add profileImage and name here
      .populate('friendRequests.sender', 'username email profileImage name');  // Add profileImage and name here
    res.json({
...
// New route to get contacts (same as friends)
router.get('/contacts', auth, async (req, res) => {
  try {
    const user = await User.findById(req.user.id)
      .populate('friends', 'username name profileImage'); // Populate friends with necessary fields
    if (!user) {
